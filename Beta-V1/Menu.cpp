...
void Menu::CreateGUI()
{
	/*This is 10 times worse than the rewrite since during V1 i had 0 idea what i was doing*/

	if (GAME_TF2) //i forgot to add the Incorrect game tab to the Rewrite so enjoy copying this over
	{
		Tabs.AddTab(
			new Tab("", {
				new DrawPanel("Incorrect game", gMenu.style->About, scale.x - TAB_WIDTH - 32, scale.y - MENU_TOPBAR)
			})
			);
	}
}
...
#pragma region Main window
	// Topbar + outline
	int topbar = style->TopBar(pos.x, pos.y, scale.x, "Oanitic");
	...
	// Tab region
	gDraw.DrawRect(_pos.x, _pos.y, TAB_WIDTH, scale.y, SColor(29, 43, 56)); //default here is 22 23 24
	gDraw.DrawLine(_pos.x + TAB_WIDTH - 1, _pos.y, _pos.x + TAB_WIDTH - 1, _pos.y + scale.y, SColor(0));
	gDraw.DrawString(_pos.x + 10, _pos.y + scale.y - 32, SColor(120, 120), "- 0.0.5 BETA - ", gFonts.verdana_bold);
