...
int DefaultStyle::DialogButton(int x, int y, int w, const char* text, bool mouseOver)
{
	...
	gDraw.DrawString(x + 3, y, mouseOver ? SColor(255, 0, 0) : SColor(125, 125, 130), text, gFonts.verdana_bold);
...
{
	// Dark topbar
	gDraw.DrawRect(x, y, w, TOPBAR, SColor(4, 114, 199, 150)); //ORIGINAL  32, 34, 37
...
void WindowStyle::About(int x, int y, int w, int h)
{
	...
	gInts.Surface->GetTextSize(gFonts.arial, L"You injected into the wrong game! this is for tf2c not tf2\n", iWidth, iHeight);
	gDraw.DrawString(CenterX - (iWidth / 2), y + 5, SColor(160), L"You injected into the wrong game! this is for tf2c not tf2\n");
