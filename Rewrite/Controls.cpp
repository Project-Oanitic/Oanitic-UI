...
#define HTEXT SColor(0, 217, 255) //0, 217, 255 original
#define CTEXT SColor(180)
#define BACKGR SColor(65, 65, 72) //65, 65, 72 original
...
int Tab::Draw(bool mouseOver)
{
	...
	if (mouseOver)
		gDraw.DrawRect(x, y, w, h, SColor(33, 56, 86, 150));
	gDraw.DrawString(x + 20, y + 2, clr, name, gFonts.calibri_light);
	if (enabled)
	{
		gDraw.DrawRect(x, y, w, h, SColor(33, 66, 96, 150));
	}
...
int Groupbox::Draw(bool mouseOver)
{
	...
	gDraw.DrawLine(x, y, x, y + h, SColor(60));
	gDraw.DrawLine(x + w, y, x + w, y + h, SColor(60));
	gDraw.DrawLine(x, y + h, x + w, y + h, SColor(60));

	gDraw.DrawString(x + (w / 2) - (strw / 2), y - (strh / 2), SColor(180), name, gFonts.verdana_bold); //x + (w / 2) - (strw / 2), y - (strh / 2)
	gDraw.DrawLine(x, y, x + (w / 2) - (strw / 2) - 5, y, SColor(60));
	gDraw.DrawLine(x + (w / 2) + (strw / 2) + 5, y, x + w, y, SColor(60));
...
{
	if (value)
		gDraw.DrawRect(x + 3, y + 3, h - 6, h - 6, SColor(29, 150, 255));

	gDraw.OutlineRect(x, y, h, h, SColor(29, 150, 255));
...
void DrawColorBox(void* data, size_t Index)
{
	...
	static Checkbox checkbox("Disabled");
